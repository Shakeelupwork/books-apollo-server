import mongoose from "mongoose";
import {randomUUID} from 'crypto';

const Schema = mongoose.Schema;

/**
 * Books schema
 * @constructor Books model constructor
 * @classdesc uuid (random and unique token. Created to provided a random identifier token for every book different than _id native MongoDB value)
 */
const imageAddress = "data:image/png;base64,"
// const defaultImage = new Buffer(imageAddress.split(",")[1],"base64");
const enumObject={
    values:['Want to Read','Reading','Read'],
    message:"valid collection status is requried"
}
const BooksSchema = new Schema({
    title:{
        type:String,
        required:true,
        unique:true,
        trim:true,
    },
    author:{
        type:String, 
        required:true,
        trim:true
    },
    uuid: {
		type: String,
		required: true,
		unique: true,
		default: randomUUID
	},
    coverImage:{
        type: String, 
        required:true,
        default:imageAddress
    },
    createdBy:{
        type: Schema.Types.ObjectId,
        ref:'users'    
    },
    // bookCollection:{
    //     type:String,
    //     required: [true, 'A book has to have a collection'],
    //     enum: enumObject,
    //     default: 'Want to Read'
    // }
},{ timestamps: true });


export {BooksSchema};